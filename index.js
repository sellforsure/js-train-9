...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex((index) => index.name === name);
  return foundIndex;
}
...
function findLastEvenNumber(arr) {
  ...
  const foundLast = arr.findLast((elem) => elem % 2 === 0);
...
function getPositiveArrayValues(arr) {
  ...
  const obj = Array.from(arr.values());
  const positiveArrayValues = obj.filter((value, index, array) => value > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const remove = arr.splice(startIdx);
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce((num, value) => num + value, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squareArray = flatArray.map((elem) => elem ** 2);
  const filteredSquaredArray = squareArray.filter((num) => num > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sliceArray = arr.slice(-n);

  const sum = sliceArray.reduceRight((num, value) => num + value, 0);
...
function expandArrayByN(arr, n) {
  ...
  const expandedArray = arr.flatMap((elem) => Array(n).fill(elem));
